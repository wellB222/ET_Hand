...
for i, port in enumerate(available_ports):
    print(f"{i+1}. {port}")
choice = int(input("Select Servo COM(num):"))
selected_port = available_ports[choice-1]
serialHandle = serial.Serial(selected_port, 115200)
...
if __name__ == '__main__':
    bus = ServoController()
    bus.stop(1)
    status = bus.get_servo_load_state(1)
    bus.unload_servo(1,status)
    time.sleep(1)
    print(bus.get_servo_id(1))
    print(bus.get_servo_position(1))